         ...
         <div class="container-fluid main-container">
             <div class="page-header">
                 ...
                 <div class="card-header d-flex justify-content-between align-items-center">
                     <a class="btn btn-primary" href="javascript:void(0)" id="create-subscription-plans-record">
                         <i class="fa fa-plus-circle" style="font-size:24px;"> Add Membership Plan</i>
                     </a>
                 </div>
             </div>
             <div class="row">
                     ...
                     <div class="card">
                         <div class="card-body p-2">
                             ...
                             <div class="table-responsive">
                                 <div id="subscription-plans-table"></div>
                             </div>
                         </div>
                     </div>
                 </div>
 ...
 @section('scripts')
     <script>
         ...
         var table = new Tabulator("#subscription-plans-table", {
             data: myData,
             layout: "fitColumns",
             columns: [{
                     title: 'ID',
                 ...
                 },
                 {
                     ...
                     hozAlign: 'center',
                     vertAlign: "middle",
                     headerFilterPlaceholder: 'Search by Plan Name'
                 },
                 {
                     title: 'Description',
                     field: 'description',
                 ...
                 },
                 {
                     ...
                     vertAlign: "middle",
                     headerFilter: "input",
                     headerFilterPlaceholder: 'Search by Email Price',
                     formatter: "money",
                     formatterParams: {
                         decimal: ".",
                         thousand: ",",
                 ...
                 },
                 {
                     ...
                     vertAlign: "middle",
                     headerFilter: "input",
                     headerFilterPlaceholder: 'Search by Post Price',
                     formatter: "money",
                     formatterParams: {
                         decimal: ".",
                         thousand: ",",
         ...
         $('#create-subscription-plans-record').click(function() {
             $.get("{{ route('subscription-plans.create') }}", form => {
                 $('#crud').html(form.html);
                 $('#crud').find(".modal").modal('show');
             });
         });

         document.getElementById('subscription-plans-table').addEventListener("click", function(e) {
             if (e.target.classList.contains("view-button")) {
                 var planId = e.target.getAttribute("data-id");
                 openViewModal(planId);
         ...
         function openUpdateModal(planId) {
             $.get("{{ route('subscription-plans.edit', ['subscription_plan' => '__planId__']) }}".replace('__planId__',
                 planId), function(response) {
                 $('#crud').html(response.html);
                 $('#crud').find(".modal").modal('show');
             });
         }

         function openViewModal(planId) {
             $.get("{{ route('subscription-plans.show', ['subscription_plan' => '__planId__']) }}".replace('__planId__',
                 planId), function(response) {
                 $('#crud').html(response.html);
                 $('#crud').find(".modal").modal('show');
             });
