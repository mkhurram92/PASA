        ...
        <div class="container-fluid main-container">
            <div class="page-header">
                <div class="page-leftheader">
                    <h3 class="page-title">Source Of Arrivals</h3>
                </div>
                <div class="card-header d-flex justify-content-between align-items-center">
                    <a class="btn btn-primary" href="javascript:void(0)" id="create-source-of-arrival-record">
                    ...
                    <div class="card">
                        <div class="card-body p-2">
                            ...
                            <div class="table-responsive">
                                <div id="sourceofarrivals-table"></div>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('scripts')
    <script>
        ...
        $('#create-source-of-arrival-record').click(function() {
        ...
        document.getElementById('sourceofarrivals-table').addEventListener("click", function(e) {
        ...
        function openUpdateModal(arrivalsource) {
            $.get("{{ route('source-of-arrivals.edit', ['source_of_arrival' => '__arrivalsource__']) }}".replace(
                '__arrivalsource__', arrivalsource), function(response) {
                $('#crud').html(response.html);
                $('#crud').find(".modal").modal('show');
            });
        }

        function openViewModal(arrivalsource) {
            $.get("{{ route('source-of-arrivals.show', ['source_of_arrival' => '__arrivalsource__']) }}".replace(
                '__arrivalsource__', arrivalsource), function(response) {
                $('#crud').html(response.html);
                $('#crud').find(".modal").modal('show');
            });
        ...
        document.getElementById("download-csv").addEventListener("click", function() {
            table.download("csv", "County List.csv");
        });

        //trigger download of data.xlsx file
        document.getElementById("download-xlsx").addEventListener("click", function() {
            table.download("xlsx", "County List.xlsx", {
                sheetName: "PASA01",
            });
        });
        ...
        document.getElementById("download-pdf").addEventListener("click", function() {
            table.download("pdf", "County List.pdf", {
