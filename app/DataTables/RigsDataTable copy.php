...
class RigsDataTable extends DataTable
{
    private $can_view, $can_edit;
    function __construct()
    {
        $this->can_view = Auth::user()->can("rigs-list");
        $this->can_edit = Auth::user()->can("rigs-edit");
    }
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($rig) {
                $html = "<div class='d-flex'>";
                if ($this->can_view) {
                    $html .= '<a href="javascript:void(0)"
                            data-href="' . route('rigs.show', ['rig' => $rig->id]) . '"
                            class="btn btn-default view-record">View</a>';
                }
                if ($this->can_edit) {
                    $html .= '<a href="javascript:void(0)"
                            data-href="' . route('rigs.edit', ['rig' => $rig->id]) . '"
     ...
     *
     * @param \App\Models\Rig $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(Rig $model): QueryBuilder
    {
