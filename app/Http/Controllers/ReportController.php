...
class ReportController extends Controller
{
    ...
    private function profitAndLoss()
    {
        ...
        // Query transactions grouped by parent gl_code, gl_code, and date
        $reportData = Transaction::select(
                ...
                DB::raw('SUM(CASE WHEN transactions.transaction_type_id = '.$incomeTypeId.' THEN transactions.amount ELSE 0 END) - SUM(CASE WHEN transactions.transaction_type_id = '.$expenditureTypeId.' THEN transactions.amount ELSE 0 END) as total_profit_loss')
            )
            ->join('gl_codes', 'transactions.gl_code_id', '=', 'gl_codes.id')
            ->join('gl_codes_parent', 'gl_codes.parent_id', '=', 'gl_codes_parent.id')
            ->groupBy(DB::raw('DATE(transactions.created_at)'), 'gl_codes_parent.name', 'gl_codes.name')
