...
class PaymentController extends Controller
{
    ...
    public function processPayment(Request $request)
    {
        ...
        try {
            // Create a charge using the token from the request
            $charge = Charge::create([
                'amount' => 5000, // Amount in cents (e.g., $50.00)
                'currency' => 'aud',
                'description' => 'Membership Renewal',
                'source' => $request->input('stripeToken'), // The token generated by Stripe.js
                'receipt_email' => $request->user()->email, // Optional, send a receipt to this email
                'metadata' => [
                    'cardholder_name' => $request->input('cardholderName'),
                ],
            ]);

            // Ensure the charge was successful
